...
import { X, Download, Trash2, BookmarkCheck, FileText, Sparkles } from 'lucide-react';
...
export const NotesDrawer: React.FC<NotesDrawerProps> = ({ isOpen, onClose, notes, onDeleteNote }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-lg bg-[#0f172a] border-l border-slate-800 h-full flex flex-col shadow-2xl overflow-hidden animate-slide-left">
        ...
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-white text-sm">Saved Academic Notes</h3>
              <p className="text-[11px] text-slate-400">{notes.length} key concepts saved</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {notes.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 text-slate-500 space-y-3">
              ...
              <p className="text-xs">No saved notes yet. Click "Save Note" on any answer card to bookmark key insights.</p>
            </div>
          ) : (
            notes.map((note) => (
              <div key={note.id} className="glass-card p-4 rounded-xl border border-slate-800 space-y-2 relative group">
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-white text-xs leading-snug">{note.topic}</h4>
                  <button
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-500 pt-1 border-t border-slate-800/50 font-mono">
                  ...
                  {note.sources && note.sources.length > 0 && (
                    <span className="text-brand-400">{note.sources.length} sources cited</span>
                  )}
                </div>
              </div>
            ))
